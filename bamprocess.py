...
# it needs reads.py and genotypes.pt in the same folder
	...
	for _group, reads in enumerate(GroupedReads):
		...
		for _barcode, _rec in linked_reads.items(): 
			try: 
				...
				frag_lst[_barcode], q_lst[_barcode] = frag_gen(varpos, allelelst, genolst[_group], coordinates, nucleotides, quals)
				_HapCUT_alleles = [[]]
				_HapCUT_starts = []
				_snp_alleles = frag_lst[_barcode].GetalleleSet(frag_lst[_barcode].GetBegin(), frag_lst[_barcode].GetEnd())
				if len([_x for _x in _snp_alleles if _x!='-'])>1: #keep only SNP-fragments that have a length of at least 2
					for _n, _allele in enumerate(_snp_alleles): # prepare records from the fragment list and the quality list to be written in HapCUT format
						if _allele == '-' and _HapCUT_alleles[-1]!=[]:
							...
							_HapCUT_starts.append(str(_n+frag_lst[_barcode].GetBegin()+1))
						elif _allele != '-':
							_HapCUT_alleles[-1].append(_allele)
					...
					HapCUT[_barcode] = str(len(_HapCUT_starts))+'\t'+'BX::'+_barcode+'\t'+'\t'.join('\t'.join([_start, _alleles]) for _start, _alleles in zip(_HapCUT_starts, _HapCUT_alleles))+'\t'+''.join(chr(int(_q)+qoffset) for _q in q_lst[_barcode].values())
					out.write(HapCUT[_barcode]+os.linesep)
		out.close()
