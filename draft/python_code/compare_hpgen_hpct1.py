...
def parse_hap_truth(file):
    """ extracting haplotype from haplotype truth file  from haplogenreator
    input: file name
    ...
    for line in hap_file:
        if line.strip().startswith("B"):
            ...
            label = int(line_slice[2])
            dic[label] = int(line_slice[5])
    return dic 
...
#    print(len(crrct_all))
    ...
    print('switch error rate',np.round(np.mean(swer_all),3))
    ...
    print('mean of block length',np.round(np.mean(leng_block_all),3))
